...
- `src/index.ts` is only the stdio MCP bootstrap.
- `src/mcp-tools.ts` is the generic MCP registrar and response formatter.
- `src/tools.ts` is the public barrel for tool functions, tool definitions, and shared types.
- `src/tools/registry.ts` owns the ordered list of registered tool definitions.
- `src/tools/definition.ts` owns the shared `MicrOSToolDefinition` type.
- `src/tools/common.ts` owns shared micrOS infrastructure: cache access, device selection helpers, TCP socket client, discovery helpers, output parsers, and concurrency helpers.
- `src/tools/<tool-name>.ts` owns one tool's input type, Zod schema, MCP title/description, exported business function, and exported tool definition.
- `src/ui.ts` and `public/` are the local tester UI and should consume MCP schemas instead of duplicating tool knowledge.
- `scripts/` contains operational entrypoints and checks. Keep scripts protocol-safe when used by stdio MCP clients.

## Adding A Tool

1. Add `src/tools/<tool-name>.ts`.
...
5. Add the definition to `src/tools/registry.ts`.
6. Export the function, input type, and definition from `src/tools.ts`.
...
- `src/mcp-tools.ts` is responsible for wrapping handler results into MCP text content and marking controlled failures as MCP errors.
...
- Keep reusable micrOS protocol behavior in `src/tools/common.ts`; keep tool-specific orchestration in the tool file.
